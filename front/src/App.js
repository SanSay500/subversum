...
const labelStyle = {
    margin: '10px 0 5px 0', fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '15px',
};
...
const App = () => {
    ...
    return (<>
        ...
        {buildings_ar === undefined ? (<></>) : (buildings_ar.map((el, i) => <div key={i}>{el.type}</div>))}

        <button style={submitStyle} onClick={users}>
            Users
        </button>
        {users_ar === undefined ? (<></>) : (users_ar.map((el, i) =>
            <div key={i}>{el.name} - {el.email}
                - regions : {el.regions.map((region, index) => {
        ...
        {planets_ar === undefined ? (<></>) : (planets_ar.map((el, i) => <div key={i}>{el.name}</div>))}
