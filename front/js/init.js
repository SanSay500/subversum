	...
	var FrenifyNeoh = {
		...
		countdown: function () {
			$('.neoh_fn_countdown').each(function () {
				...
				if (e.hasClass('boxed')) {
					e.after(
						'<div class="neoh_fn_boxed_countdown"><ul><li class="days"><div class="item"><h3></h3><span>Days</span></div></li><li class="hours"><div class="item"><h3></h3><span>Hours</span></div></li><li class="minutes"><div class="item"><h3></h3><span>Minutes</span></div></li><li class="seconds"><div class="item"><h3></h3><span>Seconds</span></div></li></ul></div>'
					);
					e.remove();
				}
				if (t === 'due_date') {
			...
			$('.subscribe_form a').on('click', function () {
				...
				if (email === '') {
					...
					if (FrenifyNeoh.validateEmail(email)) {
						$.post(
							'modal/subscribe.php',
							{ ajax_email: email, ajax_message: message },
							function (data) {
						...
						echoM
							.append('<span>' + echoM.data('invalid-email') + '</span>')
							.slideDown(500)
							.delay(4000)
							.slideUp(500);
					}
		...
		productModal: function () {
			...
			$('.neoh_fn_drops .item a')
				...
				.on('click', function () {
					...
					if (included === 'on') {
						modal
							.find('.neoh_fn_product_modal')
							.html(parent.find('.hidden_info').html());
					} else {
		...
		progressTotop: function () {
			...
			var scrolled = parseInt(
				(winScroll / (height - window.innerHeight)) * 300
			);
						...
						resultBox
							...
							.before(
								'<div class="result_item" data-id="' +
									...
									filterName +
									'</span>' +
									'<img src="svg/cancel.svg" alt="" class="fn__svg"></a></div>'
							);
		...
		removeFilter: function () {
			$('.neoh_fn_result_box .result_item a')
				...
				.on('click', function () {
					...
					resultItem.remove();
					parent
						.find('.neoh_fn_filters .checkbox[data-id="' + id + '"]')
		...
		timeLine: function () {
			...
			$.each(allProgressItems, function (i, e) {
				$(e)
					.find('a')
					.css({ left: initialSpace + spaceBetweenItems * i + 'px' });
			});

			activeLine.css({
				width:
					initialSpace +
					activeProgressItem.index() * spaceBetweenItems +
					activeProgressItem.find('a').width() / 2 +
					'px',
			});
			progressLine.css({
				width:
					initialSpace * 2 +
					spaceBetweenItems * (allProgressItems.length - 1) +
					allProgressItems.last().find('a').width() / 2 +
					'px',
			});
			...
			allProgressButtons.off().on('click', function () {
				...
				if (!p.hasClass('active')) {
					...
					activeLine.css({
						width:
							initialSpace +
							(activeIndex - 1) * spaceBetweenItems +
							e.width() / 2 +
							'px',
					});
					allContentItems.removeClass('active');
					timeline
						.find('.timeline_item[data-index="' + activeIndex + '"]')
						.addClass('active');
				}
				return false;
			});
			...
			nextButton.off().on('click', function () {
				...
				if (window.matchMedia('(max-width: 768px)').matches) {
					...
					var activeIndex =
						(parseInt(p.data('index')) + 1) % progress.find('li').length;
					if (activeIndex === 0) {
		...
		timelineClasses: function (allContentItems, allProgressItems, activeIndex) {
			...
			isActive = false;
			if (activeIndex === 'initial') {
				extraActiveIndex = $('.neoh_fn_timeline .progress_line .active').data(
					'index'
				);
			} else {
